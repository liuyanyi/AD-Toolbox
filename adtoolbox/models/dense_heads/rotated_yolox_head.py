...
class RotatedYOLOXHead(YOLOXHead):
    """RotatedYOLOXHead head used in `YOLOX <https://arxiv.org/abs/2107.08430>`_.
    ...
    def __init__(self,
        ...
        if self.seprate_angle:
            assert loss_angle is not None, 'loss_angle must be specified when seprate_angle is True'
    ...
    def _build_predictor(self):
        ...
        conv_cls, conv_reg, conv_obj = super(RotatedYOLOXHead, self)._build_predictor()
    ...
    def get_bboxes(self,
        ...
        flatten_angle_preds = [
            angle_pred.permute(0, 2, 3, 1).reshape(num_imgs, -1, self.angle_len)
        ...
        flatten_hbboxes_cxcywh = self._bbox_decode_cxcywh(flatten_priors, flatten_bbox_preds)
        flatten_decoded_angle = self.angle_coder.decode(flatten_angle_preds)

        flatten_rbboxes = torch.cat([flatten_hbboxes_cxcywh, flatten_decoded_angle.unsqueeze(-1)], dim=-1)
    ...
    def loss(self,
        ...
        faltten_decoded_angle_preds = self.angle_coder.decode(flatten_angle_preds.detach())

        flatten_rbboxes = torch.cat(
            [flatten_cxcywh_hbboxes, faltten_decoded_angle_preds.unsqueeze(-1)], dim=-1)
    ...
    def _get_target_single(self, cls_preds, objectness, priors, decoded_bboxes,
        ...
        return super(RotatedYOLOXHead, self)._get_target_single(cls_preds, objectness, priors, decoded_bboxes,
                                                                gt_bboxes, gt_labels)
